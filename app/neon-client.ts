...
function database() {
  ...
  if (!url) throw new Error("OPERATIONS_MEETING_DATABASE_URL is not set.");
  return neon(url);
}
...
export async function saveMeetingBundleClient(bundle: MeetingBundle): Promise<{ updatedAt: string }> {
  const sql = database();
  ...
  `;
  const updatedAt = new Date(String(rows[0]?.updated_at ?? new Date())).toISOString();
  return { updatedAt };
}

export async function deleteMeetingBundleClient(meetingId: string): Promise<void> {
  const sql = database();
  await ensureSchema(sql);
  await sql`DELETE FROM operations_meetings WHERE meeting_id = ${meetingId}`;
}

export async function loadMeetingBundleClient(meetingId: string): Promise<MeetingBundle | null> {
  const sql = database();
  await ensureSchema(sql);
  const rows = await sql`
...
export async function listStoredMeetingsClient(): Promise<{ id: string; date: string; status: string }[]> {
  const sql = database();
  await ensureSchema(sql);
  const rows = await sql`
